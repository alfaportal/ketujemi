...
export async function fetchInstagramPublicProfile(
    ...
    try {
      ...
      if (res.ok && data.business_discovery?.username) {
        ...
        return {
          ...
          fetch_status: "ok",
          link_valid: true,
          raw_json: data as Record<string, unknown>,
        };
      }

      if (data.error?.code === 4 || data.error?.code === 17) {
        return unsupportedFallback(profileUrl, "rate_limited", data);
      }
    } catch (err) {
  ...
  return fetchInstagramOEmbed(profileUrl, handle);
}
...
async function fetchInstagramOEmbed(
  ...
  return unsupportedFallback(profileUrl, "unsupported", {});
}
