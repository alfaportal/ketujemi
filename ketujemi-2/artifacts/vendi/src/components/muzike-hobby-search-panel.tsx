...
import {
  ...
  getMuzikeHobbyGroupLeafIds,
  getMuzikeHobbyGroupRows,
  getMuzikeHobbyLeafCategoryIds,
  getMuzikeHobbyLeafRows,
  getMuzikeHobbyTypeRows,
  muzikeSubcategoryPhoto,
...
export function MuzikeHobbySearchPanel({
  ...
  const hubTypes = useMemo(
    () => getMuzikeHobbyTypeRows(categories, hubId),
    [categories, hubId],
  );
  ...
  const subcategoryOptions = useMemo(() => {
    if (!typeId) return [];
    const groups = getMuzikeHobbyGroupRows(categories, typeId);
    return groups.flatMap((group) => {
      const leaves = getMuzikeHobbyLeafRows(categories, group.id);
  ...
  const buildParams = (): GetListingsParams => {
    ...
    if (subcategoryId) {
      ...
      const groups = getMuzikeHobbyGroupRows(categories, typeId);
      const leafIds = groups.flatMap((g) => getMuzikeHobbyGroupLeafIds(categories, g.id));
      if (leafIds.length) {
  ...
  return (
    <div className="mb-8 max-w-full overflow-hidden">
      <div className="rounded-2xl border border-gray-100 bg-white p-4 sm:p-6 shadow-sm space-y-0">
        ...
        <section className="space-y-4 pb-0" aria-label={t.mh_sec_types}>
          <div>
            <h3 className="text-base font-black text-gray-900">
              {(t as { fj_sec_pick_sub?: string }).fj_sec_pick_sub ?? t.category}
            </h3>
          ...
          <CategoryPhotoPickerGrid>
            {hubTypes.map((row) => (
              <CategoryPhotoPickerCard
                ...
                imageSrc={muzikeSubcategoryPhoto(row.slug, row.image_url, categories)}
                fallbackImageSrc={MUZIKE_HOBBY_HERO_PHOTO}
                imageAlt={row.name}
        ...
        >
          <div className="rounded-2xl border border-blue-100 bg-blue-50/30 p-4 sm:p-5 shadow-sm space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field id="mh-subcategory" label={t.subcategory}>
                <Select
                  key={typeId || "__none__"}
                  ...
                  disabled={!typeId}
                >
                  <SelectTrigger id="mh-subcategory" className={triggerClass}>
                    <SelectValue placeholder={t.all}>
                      {subcategoryId
